...
def start():
    ...
    option = getInput(inpstring="\n Enter Choice : ",datatype=int, options=[x for x in range(1,5)])
    if(option == 1):
        system('cls')
        randomize()
        print("\nThank You!")
...
def randomize():
    try:
        ...
        filename = filelist[getInput(inpstring="\nEnter file number : ",datatype=int, options=[x for x in range(1,n+1)])-1]
...
def displayInfo():
    ...
    print("Files Directory : {} ...{}".format(testfiles_folder_path, exists(testfiles_folder_path)))
    print("Output Directory : {} ... {}".format(output_files_folder, exists(testfiles_folder_path)))
    print("Default Directory : {} ... {}".format(default_dir, exists(testfiles_folder_path)))
